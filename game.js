...
══════════════════════════════════════════ */
let tapeTyping=false, tapeDone=false, tapeCharIdx=0, tapeFullText='', tapeTimer=null;
...
function playCassette(){
  tapeFullText = NIGHT_TAPES[selectedNight] || '';
  tapeCharIdx=0; tapeDone=false; tapeTyping=true;
  document.getElementById('tapeText').textContent='';
  document.getElementById('tapeHint').textContent='klik om verder te gaan';
  document.getElementById('tapeScreen').style.display='flex';
  tapeTypeStep();
}

function tapeTypeStep(){
  const el=document.getElementById('tapeText');
  if(tapeCharIdx<tapeFullText.length){
    el.textContent+=tapeFullText[tapeCharIdx];
    tapeCharIdx++;
    tapeTimer=setTimeout(tapeTypeStep,22);
  } else {
    tapeTyping=false; tapeDone=true;
    document.getElementById('tapeHint').textContent='klik om de nacht te starten';
  }
}
...
function tapeClick(){
  ...
  if(tapeTyping){
    ...
    document.getElementById('tapeText').textContent=tapeFullText;
    tapeTyping=false; tapeDone=true;
    document.getElementById('tapeHint').textContent='klik om de nacht te starten';
    return;
  }
...
function startGame(){
  ...
  ['startScreen','deathScreen','winScreen','nightIntroScreen','tapeScreen','nightCompleteScreen'].forEach(id=>document.getElementById(id).style.display='none');
  document.getElementById('jsScreen').style.display='none';
