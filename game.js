...
const FILES = {
  ...
  gijsSecret2:  'Images/GijsSecret2.png',
};
...
/* ══════════════════════════════════════════
   NACHT-INTRO CASSETTES
══════════════════════════════════════════ */
const NIGHT_TAPES = {
  1:`Ik heb zojuist een 1 gehaald voor mijn Engels proefwerk nadat ik ben vergeten te leren. Meester Gijs heeft me in de kelder gestopt en de deur dicht getimmerd. Ik laat dit bericht achter voor de volgende die wordt ontvoerd. Ademen gaat moeizaam in de kelder. Deuren en ventilatie maken overleving nog een beetje te doen. Sluit je deze te lang dan sterf je van zuurstof tekort. Meester Gijs dwaalt door de gangen. Kijk op de cameras in welke gang hij zit en sluit de juiste deur als hij plotseling van die camera verdwijnt. Om Meester Gijs bezig te houden kun je hem lokken met het geluidssysteem. Er loopt ook een andere dikke gast door de gangen heen. Hij is druk bezig met een interview en wilt absoluut GEEN geluid horen. Veel succes, je zult het zwaar nodig hebben.`,
...
/* Cassette-ambience (los van SFX, want speelt vóór initAudio) */
const tapeAmbienceAudio = new Audio(FILES.tapeAmbience);
tapeAmbienceAudio.loop = true;
tapeAmbienceAudio.volume = 0.4;
function stopTapeAmbience(){ tapeAmbienceAudio.pause(); tapeAmbienceAudio.currentTime=0; }

/* Generieke letter-voor-letter typewriter helper */
...
let G={};
const SFX={};
const continueAudio = new Audio(FILES.continueSnd);
...
function showKillerOnDeathScreen(killerSrc){
  ...
  // Killer wordt statisch en normaal weergegeven (glitch-effecten verwijderd op verzoek).
  stopKillerGlitch();
}

/* Zet de killer-afbeelding terug naar een schone, statische staat.
   (De willekeurige glitch-effecten — sprite-glitches, flikkeringen,
   vervormingen en willekeurige positie-/schaalveranderingen — zijn verwijderd.) */
...
function showFinalEnding(){
  ...
  const txt=document.getElementById('ncText');
  txt.textContent='';
  document.getElementById('gameScreen').style.display='none';
...
══════════════════════════════════════════ */
/* ══════════════════════════════════════════
   NACHT-INTRO / CASSETTE FLOW
══════════════════════════════════════════ */
let tapeSentences=[], tapeIdx=0, tapeMode='typing', tapeTimer=null;
let introPlayed=false;

function beginNightFlow(){
  if(!selectedNight) return;
  if(selectedNight===5){
    // Vanuit het hoofdmenu direct Nacht 5 spelen: altijd (100%) een willekeurig offer.
...
function proceedIntoNight(){
  ...
  showNightIntroChoice();
}

function showNightIntroChoice(){
  document.getElementById('niNightNum').textContent=selectedNight;
  document.getElementById('startScreen').style.display='none';
  document.getElementById('nightIntroScreen').style.display='flex';
}

/* ── Intro-cutscene (alleen vóór Nacht 1) ── */
function playIntroCutscene(){
  const scr=document.getElementById('introScreen'), el=document.getElementById('introText');
  scr.style.display='flex';
  typeText(el, INTRO_TEXT, 32, ()=>{
    setTimeout(()=>{ scr.style.display='none'; showNightIntroChoice(); }, 1100);
  });
}

function niChoice(wantsTape){
  document.getElementById('nightIntroScreen').style.display='none';
  if(wantsTape){ playCassette(); } else { startGame(); }
}

/* ── Cassetteband: zin-voor-zin dialoogsysteem ── */
function splitSentences(text){
  return (text.match(/[^.!?]+[.!?]*/g)||[]).map(s=>s.trim()).filter(Boolean);
}

function playCassette(){
  tapeSentences = splitSentences(NIGHT_TAPES[selectedNight] || '');
  tapeIdx=0;
  stopMenuMusic();
...
function tapeShowSentence(){
  const el=document.getElementById('tapeText'), btn=document.getElementById('tapeContinueBtn');
  btn.style.display='none';
  tapeMode='typing';
  let i=0; el.textContent='';
  (function step(){
    if(tapeMode!=='typing') return;
    if(i<tapeSentences[tapeIdx].length){
      el.textContent+=tapeSentences[tapeIdx][i];
      i++; tapeTimer=setTimeout(step,22);
    } else {
      tapeMode='waiting';
...
function tapeClick(){
  if(tapeMode==='typing'){
    ...
    document.getElementById('tapeText').textContent=tapeSentences[tapeIdx];
    document.getElementById('tapeContinueBtn').style.display='inline-block';
    tapeMode='waiting';
  }
}

function tapeContinueClick(){
  if(tapeMode!=='waiting') return;
...
function showNightComplete(){
  const n=selectedNight;
  document.getElementById('ncTitle').textContent = n===5 ? 'ONTSNAPT' : `NACHT ${n} VOLTOOID`;
...
function ncNextNight(){
  ...
  if(selectedNight===5){
    // Nacht 5 heeft altijd (100%) een offer.
    triggerGijsOfferScreen();
    return;
  }
  // Na elke andere nacht: 40% kans op precies één offer.
  if(Math.random()<0.4){
    triggerGijsOfferScreen();
  } else {
    proceedIntoNight();
...
/* Alle volledige-schermschermen ("screens") van het spel. Voordat een nieuw
   scherm wordt getoond, worden ze allemaal verborgen. Dit voorkomt dat een
   oud scherm (bv. het spelscherm) achter/voor een nieuw scherm blijft hangen,
   wat de oorzaak was van het "groot zwart scherm"-probleem bij het offer. */
const ALL_SCREEN_IDS = [
  'startScreen','nightIntroScreen','tapeScreen','introScreen',
  'nightCompleteScreen','gijsOfferScreen','deathScreen','winScreen',
  'jsScreen','gameScreen'
];
...
function triggerGijsOfferScreen(){
  // Nooit meer dan 1 offer tegelijk.
  ...
  let screen = document.getElementById('gijsOfferScreen');
  let textEl = document.getElementById('offerText');

  // Defensieve fallback: als het offer-scherm niet (correct) aanwezig is in de
  // HTML, bouwen we het hier zelf op zodat er nooit een leeg zwart scherm
  // verschijnt en de speler altijd het offer te zien krijgt.
  if(!screen || !textEl){
    screen = buildFallbackOfferScreen();
    textEl = document.getElementById('offerText');
  }

  textEl.textContent = currentOffer.text;
  screen.style.display = 'flex';
}

/* Bouwt (eenmalig) een eigen offer-scherm op als de HTML de benodigde
   elementen mist. Wordt boven alle andere UI getoond. */
function buildFallbackOfferScreen(){
  let screen = document.getElementById('gijsOfferScreen');
  if(screen && document.getElementById('offerText')) return screen;

  if(!screen){
    screen = document.createElement('div');
    screen.id = 'gijsOfferScreen';
  ...
  screen.style.cssText = `
    ...
    gap:24px;padding:32px;text-align:center;z-index:10000;
    font-family:'Share Tech Mono',monospace;color:#cc2222;
  `;
  screen.innerHTML = '';

  const textEl = document.createElement('div');
  textEl.id = 'offerText';
  textEl.style.cssText = 'white-space:pre-line;max-width:640px;font-size:1rem;line-height:1.5;';

  const btnRow = document.createElement('div');
  btnRow.style.cssText = 'display:flex;gap:16px;';

  const yesBtn = document.createElement('button');
  yesBtn.textContent = 'ACCEPTEER';
  yesBtn.style.cssText = 'padding:10px 20px;background:#330000;color:#ff4444;border:1px solid #ff4444;cursor:pointer;font-family:inherit;letter-spacing:2px;';
  yesBtn.onclick = ()=>offerAnswer(true);

  const noBtn = document.createElement('button');
  noBtn.textContent = 'WEIGER';
  noBtn.style.cssText = 'padding:10px 20px;background:#111;color:#888;border:1px solid #444;cursor:pointer;font-family:inherit;letter-spacing:2px;';
  noBtn.onclick = ()=>offerAnswer(false);
...
function offerAnswer(accepted){
  try{continueAudio.currentTime=0;continueAudio.play().catch(()=>{});}catch(e){}
  const screen=document.getElementById('gijsOfferScreen');
...
function startGame(){
  ...
  ['startScreen','deathScreen','winScreen','nightIntroScreen','tapeScreen','introScreen','nightCompleteScreen'].forEach(id=>{const el=document.getElementById(id); if(el) el.style.display='none';});
  document.getElementById('jsScreen').style.display='none';
...
/* ══════════════════════════════════════════
   GEHEIM: GijsSecret2 (0,1% kans bij opstarten)
══════════════════════════════════════════ */
/* Toont GijsSecret2.png 5 seconden lang, boven alle andere UI, en verdwijnt
   daarna automatisch vanzelf weer. Beïnvloedt de rest van de opstartprocedure niet. */
function showGijsSecret2(){
  if(document.getElementById('gijsSecret2Overlay')) return; // al zichtbaar
  const overlay = document.createElement('div');
  overlay.id = 'gijsSecret2Overlay';
...
function maybeShowGijsSecret2(){
  if(Math.random() < 0.001) showGijsSecret2();
}
...
window.addEventListener('load',()=>{
  // 0,1% kans op het GijsSecret2-geheim bij elke keer dat het spel opstart.
  // Dit blokkeert de rest van de opstartprocedure niet.
  maybeShowGijsSecret2();

  // Startknop koppelen
  document.getElementById('startBtn').addEventListener('click', beginNightFlow);
